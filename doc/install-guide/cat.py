...
BOOTSTRAP_URL = "https://github.com/CatcoinLtd/CatcoinQT/releases/download/v1.9.9/blockchain-20180212.zip"
...
def print_welcome():
    ...
    print("         |___/                                 ")
    print("")
    print_info("Catcoin masternode(s) installer v1.1")
...
def compile_wallet():
    ...
    is_compile = True
    if os.path.isfile('/usr/local/bin/catcoind'):
        print_warning('Wallet already installed on the system')
        is_compile = False

    if is_compile:
        print_info("Downloading wallet...")
        run_command("rm -rf /opt/CatcoinQT")
        run_command("mkdir /opt/CatcoinQT")
        run_command("cd /opt/CatcoinQT")
        run_command("wget https://github.com/CatcoinLtd/CatcoinQT/releases/download/v1.9.9/Catcoin-1.9.9-x86_64-linux-gnu.tar.gz")
        
        print_info("Installing wallet...")
        run_command("tar -zxvf Catcoin-1.9.9-x86_64-linux-gnu.tar.gz")
        run_command("cp Catcoin-1.9.9-x86_64-linux-gnu/catcoind /usr/local/bin/")
        run_command("cp Catcoin-1.9.9-x86_64-linux-gnu/catcoin-cli /usr/local/bin/")
        run_command("cp Catcoin-1.9.9-x86_64-linux-gnu/catcoin-tx /usr/local/bin/")
        run_command("catcoind")

def get_total_memory():
    return (os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES'))/(1024*1024)

def autostart_masternode(user):
    job = "@reboot /usr/local/bin/catcoind\n"
...
def setup_first_masternode():
    ...
    os.system('su - mn1 -c "{}" '.format("catcoind -daemon &> /dev/null"))

    print_info("Open your desktop wallet config file (%appdata%/Catcoin/Catcoin.conf) and copy your rpc username and password! If it is not there create one! E.g.:\n\trpcuser=[SomeUserName]\n\trpcpassword=[DifficultAndLongPassword]")
    global rpc_username
    global rpc_password
    ...
    if not os.path.exists('/home/mn1/.Catcoin/'):
           os.makedirs('/home/mn1/.Catcoin/')
    f = open('/home/mn1/.Catcoin/Catcoin.conf', 'w+')
    f.write(config)
    f.close()
    ...
    run_command('su - mn1 -c "{}" '.format("cd && unzip -d .Catcoin -o " + filename))

    run_command('rm /home/mn1/.Catcoin/peers.dat') 
...
def setup_xth_masternode(xth):
    ...
    run_command("rm -rf /home/mn{}/.Catcoin/".format(xth))

    print_info('Copying wallet data from the first masternode...')
    run_command("cp -rf /home/mn1/.Catcoin /home/mn{}/".format(xth))
    run_command("sudo chown -R mn{}:mn{} /home/mn{}/.Catcoin".format(xth, xth, xth))
    run_command("rm /home/mn{}/.Catcoin/peers.dat &> /dev/null".format(xth))
    run_command("rm /home/mn{}/.Catcoin/wallet.dat &> /dev/null".format(xth))
...
masternodeprivkey={}""".format(rpc_username, rpc_password, BASE_RPC_PORT + xth - 1, BASE_PORT + xth - 1, SERVER_IP, BASE_PORT + xth - 1, masternode_priv_key)
    ...
    print_info("Saving config file...")
    if not os.path.exists('/home/mn{}/.Catcoin/'):
        os.makedirs('/home/mn{}/.Catcoin/')
    f = open('/home/mn{}/.Catcoin/Catcoin.conf'.format(xth), 'w+')
    f.write(config)
    f.close()
    
    autostart_masternode('mn'+str(xth))
    os.system('su - mn{} -c "{}" '.format(xth, 'catcoind  -daemon &> /dev/null'))
    print_warning("Masternode started syncing in the background...")
...
def setup_masternodes():
    ...
    print_info("This system is capable to run around {} masternodes. To support Catcoin network only use one masternode per ip.".format(masternodes))
...
\tthe 'catcoin-cli getinfo' to get actual block number. Go to
\thttp://explorer.catcoinltd.net/ website to check the latest block number. After the
