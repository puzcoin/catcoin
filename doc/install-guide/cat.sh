#!/usr/bin/env bash
FILE=/root/mn_count
WALLET=https://github.com/CatcoinLtd/CatcoinQT/releases/download/v1.9.9/Catcoin-1.9.9-x86_64-linux-gnu.tar.gz
...
#create mnX user and copy bootstrap
	...
	sudo cp -rf ~/.Catcoin /home/mn$mncount/
	rm -rf /home/mn$mncount/.Catcoin/Catcoin.conf
	clear
		while true; do
			echo -e "Key In the port your want the CAT MN$mncount to listen on (normally 9999) and  press[ENTER]: \n"
			while true; do
				read catprtmnx
					if [[ $catprtmnx = $catprtmn1 ]]; then
	...
	sudo echo -e "rpcuser=$usrnammn1\nrpcpassword=$usrpasmn1\nrpcallowip=127.0.0.1\nrpcport=2235$mncount\nport=$catprtmnx\nserver=1\nlisten=1\ndaemon=1\nlogtimestamps=1\nmnconflock=1\nmasternode=1\nmasternodeaddr=$vpsip:$catprtmnx\nmasternodeprivkey=$privkeymnx\n" > /home/mn$mncount/.Catcoin/Catcoin.conf
	sudo chown -R mn$mncount:mn$mncount /home/mn$mncount/.Catcoin
	sudo ufw allow $catprtmnx	
	runuser -l mn$mncount -c 'catcoind'
	echo "Please wait 60 sec"
	sleep 60
	runuser -l mn$mncount -c 'catcoin-cli getinfo'
...
function mn1_summary() {
	...
	echo "mnconflock=1"
	echo "masternode=1"
	echo "masternodeaddr=$vpsip:$catprtmn1"
...
function xthmn_summary() {
	...
	echo "mnconflock=1"
	echo "masternode=1"
	echo "masternodeaddr=$vpsip:$catprtmnx"
...
#download catcoind and bootstrap
	cd /opt
	mkdir temp
	cd temp
	wget $WALLET
	...
	catcoind  > /dev/null 2>&1
	wget $BOOTSTRAP -O boot.zip
	sudo unzip -o /opt/temp/boot.zip -d ~/.Catcoin/
...
#create mn1 user and copy bootstrap
	...
	sudo cp -rf ~/.Catcoin /home/mn1/
	rm -rf /home/mn1/.Catcoin/Catcoin.conf
	clear
		while true; do
			echo -e "Key In a User Name for MN access (same as in wallet) \n"
			echo -e "You can find it in Windows wallet PC under this location\n"
			echo -e "click start and type in %appdata%/Catcoin and enter, edit Catcoin.conf file\n"
			...
			read usrpasmn1
			echo -e "Key In the port your want the CAT MN to listen on (normally 9999) and  press[ENTER]: \n"
			read catprtmn1
			echo -e "Key In the Masternode privatekey and press[ENTER]: \n"
			echo -e "This is the output from masternode genkey command: \n"
			read privkeymn1
	...
	sudo echo -e "rpcuser=$usrnammn1\nrpcpassword=$usrpasmn1\nrpcallowip=127.0.0.1\nrpcport=22350\nport=$catprtmn1\nserver=1\nlisten=1\ndaemon=1\nlogtimestamps=1\nmnconflock=1\nmasternode=1\nmasternodeaddr=$vpsip:$catprtmn1\nmasternodeprivkey=$privkeymn1\n" > /home/mn1/.Catcoin/Catcoin.conf
	sudo chown -R mn1:mn1 /home/mn1/.Catcoin
	sudo ufw allow $catprtmn1	
	runuser -l mn1 -c 'catcoind'
	echo "Please wait 60 sec"
	sleep 60
	runuser -l mn1 -c 'catcoin-cli getinfo'
	echo "Note block number, it should increase on next screen"
	sleep 30
	runuser -l mn1 -c 'catcoin-cli getinfo'
	sleep 10
...
echo -e "catcoin-cli masternode status\n"
