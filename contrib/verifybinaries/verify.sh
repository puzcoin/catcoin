...
###   catcoincore.org and catcoin.org and compares them.
...
WORKINGDIR="/tmp/catcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://catcoincore.org"
HOST2="https://catcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="catcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'catcoin-',
...
if [ $? -ne 0 ]; then
   echo "catcoin.org failed to provide signature file, but catcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "catcoin.org and catcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Catcoin Core binary release signing key installed?"
   fi
