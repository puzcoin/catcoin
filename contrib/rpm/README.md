...
The RPM spec file provided here is for Catcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://catcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Catcoin git repository](https://github.com/catcoin/catcoin) but are not part of
...
Currently only `Source10` is used, for the example `catcoin.conf` file.
...
in the catcoin git repository. At present only an SVG version of the Catcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the catcoin
...
needed when 0.12.1 is released, a proper fix is already in the Catcoin git
master branch.
...
idea to build Catcoin Core against a static version of that library compiled
...
The `catcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `catcoin-server` package. At this point, most RPM based
...
The `catcoin-server` package should have SELinux support. How to properly do
...
Catcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/catcoin/). He has been
packaging Catcoin for Fedora at least since 2012.
...
4. I renamed the `catcoin` package that contains the Qt GUI to `catcoin-core` as
that appears to be how the general population refers to it, in contrast to
`catcoin-xt` or `catcoin-classic`. I wanted to make sure the general population
