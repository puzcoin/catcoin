#!/bin/bash
# Copyright (c) 2013 The Catcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Catcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Catcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Catcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Catcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Catcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Catcoin 0.8 datadir."
    ;;
esac
