...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Catcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Catcoin.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeionamount=<n> " + _("Keep N Catcoin anonymized (default: 0)") + "\n";
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.catcoin.org>";
...
/** Sanity checks
 *  Ensure that Catcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Catcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Catcoin is probably already running."), strDataDir));
    ...
    if (fDaemon) fprintf(stdout, "Catcoin server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // catcoin: reserve balance amount
    {
    ...
    nAnonymizeCatcoinAmount = GetArg("-anonymizeionamount", 0);
    if(nAnonymizeCatcoinAmount > 999999) nAnonymizeCatcoinAmount = 999999;
    if(nAnonymizeCatcoinAmount < 2) nAnonymizeCatcoinAmount = 2;
    ...
    LogPrintf("Anonymize Catcoin Amount %d\n", nAnonymizeCatcoinAmount);

    /* Denominations
       A note about convertability. Within Stashedsend pools, each denomination
       is convertable to another.
       For example:
       1CAT+1000 == (.1CAT+100)*10
       10CAT+10000 == (1CAT+1000)*10
    */
