...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // due to a new block arriving or other large catcoin.
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Catcoin users should be well used to small numbers with many decimal places by now :)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
...
#endif
#ifdef WIN32
        int nProtLevel = 10 /* PROTECTCAT_LEVEL_UNRESTRICTED */;
        int nParameterId = 23 /* IPV6_PROTECTCAT_LEVEl */;
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Catcoin is probably already running."), addrBind.ToString());
        else
