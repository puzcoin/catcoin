...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // catcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // catcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // catcoin: track money supply and mint amount info
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // catcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // catcoin: compute stake entropy bit for stake modifier
    ...
    // catcoin: compute stake modifier
...
// ----------- instantX transaction scanning -----------
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // catcoin: check transaction timestamp
...
#ifdef ENABLE_WALLET
// catcoin: attempt to generate suitable proof-of-stake
