...
int nAnonymizeCatcoinAmount = 1000;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Catcoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "catcoin.pid"));
...
{
    std::string strCopyrightHolders = strPrefix + "The Catcoin Core developers";

    // Check for untranslated substitution to make sure Catcoin Core copyright is not removed by accident
//    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Catcoin Core") == std::string::npos) {
//        strCopyrightHolders += "\n" + strPrefix + "The Catcoin Core developers";
//    }
