...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CAT unit input in lnReqAmount, we generate the URI with CAT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CatcoinUnits::format(CatcoinUnits::CAT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
