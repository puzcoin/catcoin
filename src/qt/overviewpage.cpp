...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(CatcoinUnits::CAT)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        QRect decorationRect(mainRect.topLeft(), QSize(DECORATCAT_SIZE, DECORATCAT_SIZE));
        int xspace = DECORATCAT_SIZE + 8;
        int ypad = 6;
        ...
        painter->setPen(foreground);
        QString amountText = CatcoinUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
    ...
    {
        return QSize(DECORATCAT_SIZE, DECORATCAT_SIZE);
    }
    ...
    filter(0)
{
    ...
    ui->listTransactions->setIconSize(QSize(DECORATCAT_SIZE, DECORATCAT_SIZE));
    ui->listTransactions->setMinimumHeight(NUM_ITEMS * (DECORATCAT_SIZE + 2));
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelAnonymized->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, anonymizedBalance));
    ui->labelTotal->setText(CatcoinUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    ui->labelWatchAvailable->setText(CatcoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    ui->labelWatchStake->setText(CatcoinUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    ui->labelWatchPending->setText(CatcoinUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    ui->labelWatchImmature->setText(CatcoinUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    ui->labelWatchTotal->setText(CatcoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateStashedsendProgress()
{
    ...
    QString strAnonymizeCatcoinAmount = CatcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCatcoinAmount * COIN, false, CatcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCatcoinAmount = strAnonymizeCatcoinAmount.remove(strAnonymizeCatcoinAmount.indexOf("."), CatcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCatcoinAmount + " / " + tr("%n Rounds", "", nStashedsendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCatcoinAmount*COIN) nMaxToAnonymize = nAnonymizeCatcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCatcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCatcoinAmount));
        strAnonymizeCatcoinAmount = strAnonymizeCatcoinAmount.remove(strAnonymizeCatcoinAmount.indexOf("."), CatcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCatcoinAmount + " / " + tr("%n Rounds", "", nStashedsendRounds);
    } else {
        QString strMaxToAnonymize = CatcoinUnits::formatHtmlWithUnit(nDisplayUnit, nMaxToAnonymize, false, CatcoinUnits::separatorAlways);
        ui->labelAmountRounds->setToolTip(tr("Not enough compatible inputs to anonymize <span style='color:red;'>%1</span>,<br>"
                                             "will anonymize <span style='color:red;'>%2</span> instead")
                                          .arg(strAnonymizeCatcoinAmount)
                                          .arg(strMaxToAnonymize));
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), CatcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = "<span style='color:red;'>" +
                             QString(CatcoinUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
    ...
    if (askIfAGoodIdea == QMessageBox::Yes) {
        ...
        if(!fEnableStashedsend) {
            ...
            if(balance < minAmount) {
                QString strMinAmount(CatcoinUnits::formatWithUnit(nDisplayUnit, minAmount));
                QMessageBox::warning(this, tr("Darksend"),
