...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", CatcoinUnits::CAT);
    ...
    if (!settings.contains("nAnonymizeCatcoinAmount"))
        settings.setValue("nAnonymizeCatcoinAmount", 1000);
    nAnonymizeCatcoinAmount = settings.value("nAnonymizeCatcoinAmount").toLongLong();
    if (settings.contains("nStashedsendRounds"))
        SoftSetArg("-stashedsendrounds", settings.value("nStashedsendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCatcoinAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeCatcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCatcoinAmount:
            return QVariant(nAnonymizeCatcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case StashedsendRounds:
            ...
            break;
        case AnonymizeCatcoinAmount:
            nAnonymizeCatcoinAmount = value.toInt();
            settings.setValue("nAnonymizeCatcoinAmount", nAnonymizeCatcoinAmount);
            emit AnonymizeCatcoinAmountChanged(nAnonymizeCatcoinAmount);
            break;
        default:
            break;
        }
