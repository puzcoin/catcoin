...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    CCatcoinAddress address(scriptID);
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CCatcoinAddress(address).Get()))
        wallet->SetAddressBookName(CCatcoinAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CCatcoinAddress address(recipient.address.toStdString());
