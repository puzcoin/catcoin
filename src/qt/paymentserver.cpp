...
const int CAT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CAT_IPC_PREFIX("catcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(CAT_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CAT_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(CAT_IPC_CONNECT_TIMEOUT);
