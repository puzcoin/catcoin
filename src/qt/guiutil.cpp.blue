...
#include "catcoinaddressvalidator.h"
...
{
    widget->setMaxLength(CatcoinAddressValidator::MaxAddressLength);
    widget->setValidator(new CatcoinAddressValidator(parent));
    widget->setFont(catcoinAddressFont());
}
...
bool parseCatcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // Catcoin: check prefix
    if(uri.scheme() != QString("catcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CatcoinUnits::parse(CatcoinUnits::CAT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCatcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseCatcoinURI(uriInstance, out);
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the catcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef catcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, catcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add catcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, catcoinAppUrl, NULL, NULL);
    }
    ...
    QMessageBox(parent)
{
    ...
    coreOptions = QString::fromStdString(HelpMessage(HMM_CAT_QT));
